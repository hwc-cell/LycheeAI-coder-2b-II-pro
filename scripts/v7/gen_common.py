...
OUT_DIR = "/Users/hwc/LycheeAI/data_coder2b_pro_v6"
...
# ★ 修 2026-09-13：工具定义必须与轨迹实际调用的工具一致
#
#   原写法无条件拼 tdef(CALC)，结果有些轨迹（"查北京天气"、"读 config.py"）
#   调用的是 get_weather / read_file，而 system 里只定义了 calculate ——
#   这等于在教模型"调用没定义的工具"，正是 K2 想压制的幻觉行为。
#   实测 v7 数据集里有 9 处这种自相矛盾的样本。
#
#   修法：先扫一遍轨迹用到了哪些工具，只定义用到的那些。
_TOOL_BY_NAME = {
    "calculate": CALC, "get_weather": WEATHER, "search_web": SEARCH,
    "read_file": READF, "execute_shell": SHELL, "get_time": TIME,
    "run_python": RUNPY, "send_email": EMAIL,
}


def _tools_used(turns):
    """从轨迹里扫出所有被调用的工具定义元组（按出现顺序去重）"""
    used = []
    for r, c in turns:
        if r != "assistant":
            continue
        m = re.match(r'\s*\{\s*"name"\s*:\s*"([^"]+)"', c or "")
        if not m:
            continue
        nm = m.group(1)
        if nm in _TOOL_BY_NAME and _TOOL_BY_NAME[nm] not in used:
            used.append(_TOOL_BY_NAME[nm])
    return used


# ★ 干扰工具：让「多工具选择」这个能力有训练信号
#
#   只定义轨迹用到的那个工具，模型学到的其实是
#   "system 里有什么就调什么"（1 个工具时无需选择）。
#   实测这么改之后，"见过 ≥3 种工具"的样本从 56 条掉到 0 条，
#   模型将失去「在多个候选里挑对的那个」的训练信号。
#
#   所以按 1:2 的比例：三分之一保持单工具（对应极简宿主），
#   三分之二掺入 1~3 个无关工具做干扰项 —— 但不掺 calculate，
#   否则模型会学到"有 calculate 时优先调它"，污染其他工具的样本。
_K4_DISTRACT = [WEATHER, SEARCH, READF, TIME, SHELL, RUNPY]


def _k4_head(sys_text, turns, variant):
    """构造 K4 样本的 system：按 variant 决定掺不掺干扰工具"""
    used = _tools_used(turns)
    if not used:
        return sys_text
    if variant == 0:
        # 单工具：对应"宿主只给一个工具"的极简场景
        return sys_text + "\n\n" + tdef(*used)
    # 掺干扰项：从没用到的工具里随机挑 1~3 个
    pool = [t for t in _K4_DISTRACT if t not in used]
    k = min(len(pool), 1 + (variant % 3))
    extra = pool[:k]
    # 工具顺序也要打乱 —— 否则模型学到"第一个就是答案"
    allt = list(used) + extra
    _rng_k4.shuffle(allt)
    return sys_text + "\n\n" + tdef(*allt)


_rng_k4 = random.Random(31337)

for turns, weight in K4_TRAJECTORIES:
    for vi, sys_text in enumerate(GENERIC_SYS[:3]):
        real = [(r, c) for r, c in turns]
        head = _k4_head(sys_text, real, vi)
        conv = [("system", head)] + real
